...
| 부품                 | 모델명               | 하는 일                          | 연결 방식          |
| -------------------- | -------------------- | -------------------------------- | ------------------ |
| **두뇌 (MCU)** | ESP32-S3 DevKitC-1   | 모든 계산을 담당하는 소형 컴퓨터 | -                  |
| **GPS**        | GY-GPSV3-NEO-M8N     | 위성에서 신호 받아 위치 계산     | UART (문자로 대화) |
| **IMU**        | GY-9250 (MPU-9250)   | 움직임 감지 (가속도+회전+나침반) | I2C (번호로 대화)  |
| **심박**       | MAX30102 Module      | 손목 혈관에 빛 쏴서 맥박 측정    | I2C                |
| **배터리**     | 3.7V 500mAh LiPo     | 전원 공급 (3.7V)                 | -                  |
...
```
ESP32-S3 DevKitC-1 핀 배치:
┌─────────────────────────────────────┐
│           ESP32-S3 DevKitC-1        │
│                                     │
│  GPIO 21 (SDA) ────┬──── GY-9250 SDA
│  GPIO 22 (SCL) ────┼──── GY-9250 SCL
│                    │
│                    ├──── MAX30102 SDA
│                    └──── MAX30102 SCL
│                                     │
│  GPIO 16 (RX) ────────── NEO-M8N TX │
│  GPIO 17 (TX) ────────── NEO-M8N RX │
│                                     │
│  3.3V ─────────────────── 센서 VCC  │
│  GND ──────────────────── 센서 GND  │
└─────────────────────────────────────┘
